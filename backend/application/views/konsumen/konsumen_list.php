            ...
            foreach ($konsumen_data as $konsumen)
            {
                ...
                <tr>
                    ...
                    <?php if($konsumen->activated=='') {?>
                        <td width="70px"><?php echo anchor(site_url('konsumen/app_konsumen/'.$konsumen->id),'Activated',['class'=>'btn btn-sm btn-warning']);?></td>
                    <?php }else{?>
                        <td width="50px"><?php echo anchor(site_url('konsumen/btl_konsumen/'.$konsumen->id),'Batal',['class'=>'btn btn-sm btn-danger']);?></td>
                    <?php }?>
                    <td style="text-align:center" width="220px">
                        <?php 
                        echo anchor(site_url('konsumen/read/'.$konsumen->id),'Read',['class'=>'btn btn-sm btn-primary']); 
                        echo ' | '; 
                        echo anchor(site_url('konsumen/update/'.$konsumen->id),'Update',['class'=>'btn btn-sm btn-success']); 
                        // echo ' | '; 
                        // echo anchor(site_url('konsumen/delete/'.$konsumen->id),'Delete',['class'=>'btn btn-sm btn-danger',
